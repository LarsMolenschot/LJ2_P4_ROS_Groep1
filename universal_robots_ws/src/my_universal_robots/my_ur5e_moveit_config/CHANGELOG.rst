^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package my_ur5e_moveit_config
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
1.2.1 (2018-01-06)
------------------
* Reduce longest valid segment fraction to accomodate non-limited version of the ur5e (`#266 <https://github.com//ros-industrial/universal_robot/issues/266>`_)
...
* apply latest setup assistant changes to ur5e and ur10
* Adding comment explaining the choice of default planning algorithm
* Use RRTConnect by default for ur5e
...
* new moveit_configs for ur5e and ur10
* remove old my_ur5e_moveit_config
* Contributors: Florian Weisshardt, ipa-fxm

* ur5e_moveit_cfg: add missing run_depend ind_rob_simulator. Fix `#38 <https://github.com/ros-industrial/universal_robot/issues/38>`_.
...
* Removed ur5e_joint_limited_moveit_config.  my_ur5e_moveit_config now has limited joint ranges to plus/minus 180 degrees.
* Added ur5e moveit library.  The Kinematics used by the ur5e move it library is unreliable and should be replaced with the ur_kinematics
